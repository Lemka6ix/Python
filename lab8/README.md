...
class InvalidIngredientError(RecipeError): pass
```
* **Базовое исключение** для всех ошибок рецептов

* **Конкретные исключения** для разных ситуаций

#### 2. Класс Recipe - модель рецепта
```python
class Recipe:
    def __init__(self, name: str, ingredients: List[str], instructions: str):
        self.name = name
        self.ingredients = ingredients  # список ингредиентов
        self.instructions = instructions  # инструкции приготовления
```
* Хранит данные одного рецепта

* ``matches_ingredients()`` - проверяет, содержит ли рецепт ВСЕ указанные ингредиенты

#### 3. Класс RecipeCatalog - управление данными
```python
class RecipeCatalog:
    def __init__(self):
        self.recipes: List[Recipe] = []
        self.load_recipes()  # загружает из файла при старте
```
* `add_recipe()` - добавляет новый рецепт

* `remove_recipe()` - удаляет рецепт
* `find_recipes_by_ingredients()` - ищет рецепты по ингредиентам
* `save_recipes()/load_recipes()` - сохраняет/загружает в JSON файл

#### 4. Класс RecipeApp - GUI приложение
```python
class RecipeApp:
    def __init__(self):
        self.catalog = RecipeCatalog()  # создает каталог
        self.new_recipe_ingredients = []  # ингредиенты для нового рецепта
        self.search_ingredients = []      # ингредиенты для поиска
        self.found_recipes = []           # найденные рецепты
```
...
* Поле "**Ingredient**" + кнопка "**Add**" - добавляет ингредиенты по одному

* Список "**Ingredients**" - показывает добавленные ингредиенты

* Кнопка "**Remove Selected**" - удаляет выбранный ингредиент

* Поле "**Instructions**" - многострочное поле для инструкций

* Кнопка "**Save Recipe**" - сохраняет рецепт
...
* Список "**Search by ingredients**" - показывает ингредиенты для поиска

* Кнопка "**Remove Selected**" - удаляет ингредиент из поиска

* Кнопка "**Find Recipes**" - ищет рецепты

* Список "**Found Recipes**" - показывает найденные рецепты

* Кнопка "**Delete Selected**" - удаляет выбранный рецепт
...
* Отображает название, ингредиенты и инструкции
...
def add_ingredient(self, sender, app_data):
    ...
    if ingredient and ingredient.strip():
        self.new_recipe_ingredients.append(ingredient.strip())
        dpg.configure_item("ingredients_list", items=self.new_recipe_ingredients)
        dpg.set_value("new_ingredient", "")  # очищаем поле ввода
```
...
def search_recipes(self, sender, app_data):
    try:
        recipes = self.catalog.find_recipes_by_ingredients(self.search_ingredients)
        self.found_recipes = [recipe.name for recipe in recipes]
        dpg.configure_item("found_recipes_list", items=self.found_recipes)  # обновляем интерфейс
    except RecipeError as e:
        self.show_error(str(e))
```
...
def save_recipe(self, sender, app_data):
    try:
        ...
        recipe = Recipe(name.strip(), self.new_recipe_ingredients.copy(), instructions.strip())
        self.catalog.add_recipe(recipe)  # сохраняет в файл автоматически
```
## Работа с данными
...
#### Логика поиска
* Ищет рецепты, которые содержат **хотя бы один** из указанных ингредиентов

* Регистр не важен ("Tomato" = "tomato")
